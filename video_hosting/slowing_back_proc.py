...
from backends import change_slowing

parameters = sys.argv[1:]
@shared_task
def slowing(video_path):
    print(video_path)
    a = str(video_path).split('/')[2] 
    if a.endswith('.mp3'):
        new_clip = AudioFileClip(f'{BASE_DIR}/media/{video_path}')
        new_clip = new_clip.fx(vfx.speedx, 0.5)
        new_name = str(video_path).split('/')
        new_name1 = new_name[1]
        old_name = new_name[2]
        new_clip.write_audiofile(f'{BASE_DIR}/media/slowing/{new_name1}/_{old_name}', 
        fps=None,
            nbytes=2,
            buffersize=2000,
            bitrate=None,
        ...
        change_slowing(old_name, new_name1, video_path)
        new_clip.close()
    else:
        ...
        old_name = new_name[2]
        new_clip = new_clip.fx(vfx.speedx, 0.5)
        new_clip.write_videofile(f'{BASE_DIR}/media/slowing/{new_name1}/_{old_name}',
                                                       threads=2,
                        fps=50, 
                        verbose=False,
                        logger="bar",
